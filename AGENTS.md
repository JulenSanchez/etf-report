...
用户输入含以下模式时，是**询问意图**，不是执行指令：

- "你打算…" / "你准备…" / "你想…" / "你建议…"
- "真的是…吗" / "是不是…" / "对吗" / "现在是这样的吗"
- "为什么…" / "怎么回事" / "哪个" / "在哪里"

AI 必须只回答问题，不执行写操作（Update/Edit/Write）。执行前必须等用户明确指令（"做吧"/"可以"/"执行"/"改"）。

**禁止"建议即执行"**：AI 说"建议…"或"要不要…"时是提议而非授权。禁止在说"建议"的同时执行 Update/Edit/Write。用户确认后才动手。
...
触发词：`盘中回测` / `今天的数据` / `跑一下今天的` / `看看今天` / `最近的行情`

固定参数、固定流程，详见 `docs/runbook/v2-quant/tuner.md`。AI 不得自行更改窗口或 preset。
...
学到新东西、发现文档缺口、被用户纠正行为时，按此顺序判断写到哪：

1. 工程铁律（跨会话不变） → `.claude/rules/`（见 `README.md`）
2. AI 会话行为（路由、触发词、验证） → `AGENTS.md`（本文件）
3. 运维操作（怎么排障、怎么发布、API 怎么调、真实交互流程） → `docs/runbook/`（见 `workflows.md`）
4. 设计理念（系统为什么这样设计、架构原则、术语定义） → `docs/design/`（见 `overview.md`）
5. 项目治理（看板规则、ID 申请、REQ 模板） → `PLAN.md`
6. 需求/bug 跟踪（要做什么） → `plans/Board.md`
7. 研究证据（发现了什么） → `research/`（见 `README.md`，下属 `params/` `pool/` `strategy/` `promoted/` `baselines/`）
8. 人类入口（命令、结构） → `README.md`

各层入口文件已自解释其职责。**不要遇到问题就往最近的地方塞。**
