...
触发词："优化 <preset>"。70 trials 6Y，~45 分钟。

| 参数 | 搜索范围 | 说明 |
|------|---------|------|
| w1 | [bl-10, bl+10] 步长 1 | F1 趋势偏离权重。越高越依赖趋势信号 |
| w3 | [bl-10, bl+10] 步长 1 | F3 量比权重。越高越看成交量 |
| ma_bear_pos | 赌徒 [0.3, 1.0] / 禅修 [0.15, 0.5] / 精算 [0.15, 0.30] | 熊市仓位。赌徒追求绝对收益敢接飞刀 / 禅修控回撤偏保守 / 精算低风险上限 30% |
| max_holdings | [1, 8] 步长 1 | 最大持仓数。越小越集中，1=单吊 |
| disc_step | [0.03, 0.15] 步长 **0.01（整数百分比）** | 仓位离散化步长。必须整数百分比如 0.03/0.04/0.05，禁止 0.0368 |
...
触发词："精细优化 <preset>"。100 trials 6Y，~70 分钟。快速模式 + 以下 6 个：

| 参数 | 搜索范围 | 说明 |
|------|---------|------|
| ma_trend_period | [bl-5, bl+15] 步长 1 | MA 趋势计算周期（周）。越大越慢，过滤短期波动 |
| f1_sensitivity | [bl*0.5, bl*2.0] | F1 sigmoid 陡峭度。越大好坏分化越极端 |
| f3_sensitivity | [bl*0.5, bl*2.0] | F3 log-sigmoid 陡峭度 |
| score_band | [0.5, 8.0] 步长 **0.1** | 分数带（%）。Top 候选与第一名差距在此范围内才能入选。越小越挑剔 |
| concentration | [bl*0.5, bl*2.0] | 仓位集中度 C。越高越偏向前几名 |
| c_sensitivity | [bl*0.5, bl*2.0] | C 动态灵敏度。越高对分数分散度越敏感 |
...
1. `research/params/<preset>-<date>/analysis.json`（优化器自动生成，无需手动运行）
