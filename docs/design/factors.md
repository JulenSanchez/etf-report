...
hold 的效果不是"规避假信号"，而是**信息质量保护**：在信息不够的时候，宁可用上周的完整 5/5 值，也不用一个高噪声近似去干扰 F7 的日线判断。
...
**F3（方向性量比）** 于 2026-07 v3.15.0 退役。6Y 回测空白对照实验确认其信号价值≈0（真实 F3 vs 空白常数 50，AR 差 0.13pp）。其历史作用——得分尺度调节器（20% 权重使综合分 σ 缩小，间接收紧 band）——已被 `base_score` 参数显式化。详见 `plans/REQ-393.md`。
