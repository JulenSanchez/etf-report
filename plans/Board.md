...
| BUG-029 | F1 lookahead: intraday 数据合并 → rebuild_weekly → 周bar日期漂移 → searchsorted 命中不同周 | 🔴 Critical | closed | v3.4.0 | REQ-265 | 2026-06-03 | `_get_weekly_with_cache()` 用含未来日期的日线重建周线→周bar日期漂移。修复：all_weekly永不用intraday重建。详见 plans/BUG-029.md | v3.5.0 |
