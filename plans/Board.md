...
| ID | 标题 | 完成日期 |
|----|------|---------|
| BUG-043 | **前沿点极端参数 NaN 崩溃** — idxmax() 全零返回 NaN 导致 KeyError, 已加防护 | 2026-07-07 |
| REQ-357 | **因子缓存管理 + 周线/成交量切换** — 三级数据层, 因子可视化, 缓存增删重建 | 2026-07-06 |
| BUG-042 | **因子缓存过期** — 份额变动后缓存未失效导致F1/F7异常, 已修复+因子缓存管理 | 2026-07-06 |
| REQ-355 | **数据管理面板** — ETF×日期覆盖矩阵, 增删补操作, 盘中/停牌/异常可视化 | 2026-07-06 |
| REQ-354 | **拆股自动检测+内存清洗+盘后重拉** — AKShare fund_cf_em 检测，自愈清洗 | 2026-07-06 |
| REQ-348 | **盘后定价交易 — 信号推送时间迁移** — 14:50→15:10 | 2026-07-03 |
| REQ-343 | **Tuner 进度条时间匹配** — 动态权重按实测分配 | 2026-07-02 |
| REQ-345 | **Tuner UI 设计文档** — `tuner-ui.md`（布局+交互+配色+组件） | 2026-07-02 |
| REQ-346 | **净值涨跌分布 UI** — 翻页直方图 + 统计卡片 + 极值标注 | 2026-07-02 |
