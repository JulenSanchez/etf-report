...
| 日期 | ID | 标题 | 状态 |
|------|----|------|------|
| 2026-07-22 | REQ-390 | GitHub Actions 远端部署 — 收盘前信号兜底 | ✅ 已修复+实盘验证（推送仓位正确）；BUG-066 cron 延迟另案 |
...
| 2026-07-23 | **逃顶机制 — 独立于三因子的回落保护** — gam-0 策略结构性地无法捕捉冲顶回落（暴涨当天 F1 强烈看多，F7 无反应）。需基于 NAV 回报速度的轻量逃顶触发。**2026-07-29 证伪关闭**：三组统计（单日/多日累计/MA20偏离/机会成本）一致证明大涨日后回报仍为正，逃顶假设不成立。详见 REQ-391。 | 已关闭（证伪） | → REQ-391 ✅ |

| 2026-07-24 | **三因子简化研究** — 用户盘感驱动：F3 几乎不贡献、F1/F7 交替主导、策略本质可能是"带保险杠的单因子"。**2026-07-30 关闭**：F3 正式退役，gam-0 改为 82/18 两因子 + base_score。空白对照实验 + band 缩放等价性证明 F3 信号价值 ≈0，其历史作用仅为得分尺度调节器，已被 base_score 替代。成交量信息有价值（588010 放量 6.8x +32.8%），但稀疏事件型信号不适合日频排名框架——将以仓位放大器/调仓触发器/风险过滤器等形式召回。 | 已关闭 | → REQ-393 ✅ F3 退役 |
...
| 日期 | ID | 标题 |
|------|----|------|
| 2026-07-30 | REQ-393 | F8 成交量因子重设计 → F3 退役 · gam-0 82/18 两因子化 |
| 2026-07-29 | REQ-391 | 逃顶机制 · 证伪关闭 |
| 2026-07-24 | REQ-392 | 术语表分层治理 |
...
| **v3.15.0** | Two-factor simplification + F3 retirement | ✅ F3 退役 + base_score · ✅ REQ-391 逃顶证伪 · ✅ REQ-392 术语表分层 · ✅ BUG-063/064 修复 · ⏳ REQ-390 远端兜底（Tencent fallback 待盘中验证） |
...
| BUG-066 | **GitHub Actions 收盘前信号推送延迟 ~3h** — cron 有队列延迟（~3h），实际 17:28 才执行。修复：腾讯云 SCF 定时触发器 14:45 CST → workflow_dispatch API（无队列延迟，热缓存推送 ~14:46）。详见 `docs/runbook/v2-quant/daily-automation.md`「远端定时触发」。 | 🟡 Medium | fixed | v3.15.0 | REQ-390 | 2026-08-07 | GitHub Actions 免费计划 cron 无执行时间 SLA | v3.15.0 |
| BUG-067 | **远端信号推送仓位不对** — 收到(远端)推送但仓位错误（107%+73%=180% 牛市，正确 60%+40%=100% 熊市）。双重根因：(1) cache key `github.run_date` 非法 → key 空 → 缓存冻结在 07-24（仅390KB）；(2) 冷启动无全量拉取 → 新浪快速路径写 1 行数据骗过 gap 检测。修复：cache key 改用 shell 日期 + 缓存未命中时先 `--full` 全量拉取。已验证：dry-run + 实盘推送仓位均正确（A股创新药 60% + 纳指科技 40%）。 | 🟡 Medium | fixed | v3.15.0 | REQ-390 | 2026-08-17 | cache key 空 + 冷启动无全量拉取 | v3.15.0 |
