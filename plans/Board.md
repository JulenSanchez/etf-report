...
| BUG-066 | **GitHub Actions 收盘前信号推送延迟 ~3h** — cron 有队列延迟（~3h），实际 17:28 才执行。修复：腾讯云 SCF 定时触发器 14:48 CST → workflow_dispatch API（无队列延迟，热缓存推送 ~14:49）。详见 `docs/runbook/v2-quant/daily-automation.md`「远端定时触发」。 | 🟡 Medium | fixed | v3.15.0 | REQ-390 | 2026-08-07 | GitHub Actions 免费计划 cron 无执行时间 SLA | v3.15.0 |
